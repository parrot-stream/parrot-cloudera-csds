...
if [[ "$2" == "kafka-topics-ui" ]]; then
  ...
  while read -r host; do
    ...
    PARROT_STREAM_SSL_REST_PORT=$(extract_peer_config_value $HOSTNAME "parrot-stream" "ssl.rest.port")
    if [[ ${PARROT_STREAM_SSL_ENABLED} == "true" ]]; then
      CONNECT_URL="$CONNECT_URL,https://$host:$PARROT_STREAM_REST_PORT"
    else
      CONNECT_URL="$CONNECT_URL,http://$host:$PARROT_STREAM_SSL_REST_PORT"
    fi
  done <<< "$HOSTNAMES"
  export CONNECT_URL=${CONNECT_URL#","}
  echo -e "# PARROT_STREAM_SSL_ENABLED:       $PARROT_STREAM_SSL_ENABLED"
  echo -e "# PARROT_STREAM_PORT:              $PARROT_STREAM_REST_PORT"
  echo -e "# PARROT_STREAM_SSL_REST_PORT:     $PARROT_STREAM_SSL_REST_PORT"
...
if [[ ${SSL_ENABLED} == "true" ]]; then
  ...
  perl -pi -e "s#\#tls self_signed#tls self_signed#" $CADDYFILE
else
